...
- Классификация отходов по текстовому описанию на основе TF-IDF взвешивания (634 ключевых слова)
- Прогноз загруженности экопунктов по часу дня — полиномиальная регрессия на sklearn
- Расчёт сэкономленного CO₂ на основе методологии EPA + EEA
- Интерактивная карта кампуса на базе Leaflet и 2GIS
- Telegram-бот с inline-навигацией и автоматической классификацией
- Аналитика использования: графики типов отходов и популярных локаций
...
│   ├── predictor.py        # ML прогноз загруженности (sklearn, линейная регрессия)
│   ├── stats.py            # Сервис статистики запросов + расчёт CO₂
│   ├── predictor.py        # ML-прогноз загруженности (sklearn Pipeline)
...
└── tests/
    ...
    └── test_predictor.py    # Unit-тесты ML модели (регрессия)
...
### Линейная регрессия (прогноз загруженности)

Модуль `predictor.py` использует полиномиальную регрессию (sklearn `Pipeline` +
`PolynomialFeatures` + `LinearRegression`) для предсказания загруженности
экопунктов по часу дня. Модель обучается при старте приложения на синтетических
данных суточного ритма кампуса и возвращает:

- Прогноз числа посетителей на каждый час (0–23)
- Текстовую метку: Свободно / Умеренно / Оживлённо / Пик
- Топ-3 рекомендованных часа для посещения
- Метаданные модели: R², степень полинома, число обучающих примеров

### YandexGPT (эко-ассистент)

Чат-бот на базе YandexGPT Lite API отвечает на вопросы об экологии и
раздельном сборе в свободной форме. Контекст системного промпта ограничивает
ответы экологической тематикой кампуса.
...
| GET | `/stats` | Статистика использования + CO₂ |
| GET | `/predict/load` | ML-прогноз загруженности по часу дня |
| GET | `/eco-tip` | Случайный эко-совет |
| GET | `/qr` | QR-код для быстрого доступа |

## Качество кода

- 102 теста (unit + интеграционные)
- Покрытие 82%
- CI/CD через GitHub Actions — тесты запускаются автоматически на каждый push
...
| Тариф | Аудитория | Цена |
|---|---|---|
| Стартовый | до 5 000 человек | 30 000 ₽/год |
| Университет | до 30 000 человек | 80 000 ₽/год |
| Корпорация | свыше 30 000 человек | 150 000 ₽/год |
...
| Контракт | Объём | Стоимость |
|---|---|---|
| Район | 1–5 объектов | 100 000 ₽/год |
| Округ | 5–20 объектов | 350 000 ₽/год |
| Город | 20+ объектов | от 800 000 ₽/год |
...
| Тариф | Цена |
|---|---|
| Базовый (до 500 чел.) | 1 900 ₽/мес |
| Бизнес (до 2 000 чел.) | 4 900 ₽/мес |
| Enterprise | по запросу |

### Финансовая модель

**Рынок (Россия):**
- 741 университет × средний контракт 80 000 ₽ = **TAM ~59 млн ₽/год**
- 50 крупных городов с программами раздельного сбора = **TAM B2G ~40 млн ₽/год**
- Реалистичный охват 3% в первые 2 года = **SAM ~3 млн ₽/год**

**Точка безубыточности:**
- Операционные расходы: хостинг ~3 000 ₽/мес, API YandexGPT ~2 000 ₽/мес
- **Итого: ~60 000 ₽/год**
- Достигается при **1 университетском контракте** или **3 SaaS-подписках**

**Прогноз выручки (год 1):**
- 3 университета × 80 000 ₽ = 240 000 ₽
- 10 SaaS-клиентов × 4 900 ₽ × 12 = 588 000 ₽
- **Итого: ~828 000 ₽/год при минимальных затратах**
